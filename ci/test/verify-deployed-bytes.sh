...
# The paths to compare. The two wasm modules come from the descriptor, so a
...
m = re.search(r"id=\"codetracer-deployment\"[^>]*>(.*?)</script>", html, re.S)
d = json.loads(m.group(1))
paths = ["/", "/noir", "/ui.js", "/assets/wasm-worker.js",
...
while IFS=$'\t' read -r path _ _; do
	...
	# A digest in the name is the only thing that makes `immutable` true.
	# Mirrors `assetIsContentAddressed` in web_deployment.nim. Deliberately
	# NOT a dotfile-style blanket rule: `app.9f2b1c.js` earns it, `app.v2.js`
	# and `noir_wasm.wasm` do not.
	name="${path##*/}"
...
echo
if [ "$fail" -eq 0 ]; then
	echo "every declared host serves the origin's bytes, and no stable name is immutable."
else
