...
#
# The excluded names are READ FROM `store-exclusions.json`, not hard-coded, so
# adding an exclusion to that file starts enforcing it against the tree instead
# of only being asserted to be documented.
#
# AND THE PREDICATE IS PROVEN ABLE TO FAIL FIRST. `Test-Path` against a name
# that was never created is true by construction, so on its own this pair of
# assertions could not fail no matter what the store contained -- the same
# defect that was found in two other controls in this campaign, and it is not
# worth having a third. The control below runs the IDENTICAL predicate against
# a component that IS in the store and requires the opposite answer.
$excludedNames = @((Get-Content -LiteralPath $exclusionsPath -Raw | ConvertFrom-Json).exclusions |
  ForEach-Object { ([string]$_.component).ToLowerInvariant() })
Assert-True -Condition ($excludedNames.Count -ge 2) `
  -Message "the exclusion list names at least FPC and MSVC, so the absence checks have subjects ($($excludedNames -join ','))"

Assert-True -Condition (Test-Path -LiteralPath (Join-Path $rootTwo $components[0].name)) `
  -Message "CONTROL: the same presence predicate finds '$($components[0].name)', which IS in the store -- so 'absent' below is a measurement, not a tautology"
