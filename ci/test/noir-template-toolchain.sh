...
# CHECKOUT's release build ahead of it on PATH, and the two can be different
# compilers with different answers.
#
# THE EXAMPLE THAT MADE THIS RULE HAS SINCE BEEN CLOSED, and it is left here
# because the rule outlives it. Until 2026-09-03 the flake pin was `5e98f904`
# (beta.2), which carried the lexer's `NonAsciiComment` check, while the
# sibling build measured on 2026-09-02 (`906af2f4`, beta.26) did not — upstream
...
# pin is not; they diverge again the moment someone builds a branch. So the
# gate keeps printing the resolved path and revision, and a green run that does
# not name the binary remains a result nobody can reproduce. "Measured locally"
# is not a provenance.
#
...
#                                 check that decides whether the pane's number
#                                 is right. Without it the shipped count is not
#                                 checked at all.
...
if [ -z "${CT_NOIR_WASM_COMPILER:-}" ] || [ ! -f "${CT_NOIR_WASM_COMPILER:-}" ]; then
	...
	# beta.2 lexer did — see the note in the header. The shim is now the only
	# way to produce that divergence, because the pin that used to produce it on
	# its own has moved past `93bb72f8`; that makes the shim MORE necessary, not
	# less, since nothing reachable reddens this arm by accident any more.
fi
echo
