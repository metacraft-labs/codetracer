...
if [ "$(uname -s)" != "Linux" ]; then
	echo "reprobuild Linux smoke must run on Linux; got $(uname -s)" >&2
	exit 1
fi

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
fixture_root="${repo_root}/tests/fixtures/reprobuild-linux-smoke"
expected_output="reprobuild linux smoke: hello"
...
printf '%s\n' "${capabilities_json}" >"${TMPDIR:-/tmp}/reprobuild-linux-smoke-capabilities.json"
if ! printf '%s\n' "${capabilities_json}" |
...
tmp_root="$(mktemp -d "${TMPDIR:-/tmp}/reprobuild-linux-smoke.XXXXXX")"
cleanup() {
	status=$?
	if [ "${status}" -ne 0 ] && [ "${REPROBUILD_LINUX_SMOKE_KEEP_TMP:-0}" = "1" ]; then
