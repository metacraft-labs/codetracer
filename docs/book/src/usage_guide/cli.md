...
1. `ct record <application>` - Creates a trace file that can be loaded later or shared. On Windows, recordings use TTD via `ct-rr-support record` (requires an elevated shell).
