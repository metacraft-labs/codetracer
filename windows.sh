...
nim \
    ...
    -d:debug -d:asyncBackend=asyncdispatch                 --gc:refc                 --debugInfo --lineDir:on                 --boundChecks:on --stacktrace:on --linetrace:on                 -d:chronicles_sinks=json -d:chronicles_line_numbers=true                 -d:chronicles_timestamps=UnixTime                 -d:ctTest -d:testing --hint[XDeclaredButNotUsed]:off                 -d:builtWithNix                 -d:ctEntrypoint                 --nimcache:nimcache                 --out:ct c ./src/ct/codetracer.nim
