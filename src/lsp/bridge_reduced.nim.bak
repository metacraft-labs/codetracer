...
proc onGenericNotification(connection: JsObject; handler: proc(methodName, params: JsObject) {.closure.}): JsObject {.importjs: "#.onNotification(#)".}
...
proc registerLspNotificationHandler*(handler: LspNotificationHandler) =
  ## Register a handler invoked for every notification received from the language server.
  if handler.isNil:
    return
  notificationHandlers.add(handler)

proc clearLspNotificationHandlers* =
  ## Remove all registered notification handlers.
  notificationHandlers.setLen(0)

proc dispatchNotification(methodName, params: JsObject) =
  for handler in notificationHandlers:
    try:
      handler(methodName, params)
    except CatchableError:
      let message = getCurrentExceptionMsg()
      logWarn2(toJs("LSP bridge notification handler failure"), toJs(message.cstring))
