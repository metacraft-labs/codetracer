...
proc paneHomesForMode*(mode: LayoutMode): seq[seq[int]] =
  ...
  ##
  ## * **TESTS with FILES, in BOTH modes.** The two are read together — a test
  ##   names a file, and the file is what you open next — and FILES is the pane
  ##   both modes keep in a narrow side column, so the tab costs no width in
  ##   either. It is `Content.Filesystem` and not `Content.VCS` even though they
  ...
  ##
  ##   **THE COST OF THIS ROW IS REAL, WAS MEASURED, AND IS ACCEPTED.** A nested
  ##   pane is a background tab; a background tab is `display: none`, so it is
  ##   not laid out and its controls are not hit-testable until the user clicks
  ##   the tab. On the editing surface that pane carries the ▶ Run Tests
  ##   control. When this row was first written for both modes, the pre-publish
  ##   mount gate refused the bundle over exactly that: the panes rendered at
  ##   19.8 / 54.4 / **0** where the layout declared 20/55/25, and the ▶ was
  ##   painted but took no click.
  ##
  ##   That is a statement about what nesting DOES, and it remains true. It was
  ##   raised as a trade and decided: TESTS belongs with FILES and VCS in both
  ##   modes, and the spec follows the product rather than the other way round.
  ##   `Planned-Features/Noir-Studio.md` §1a now draws it there, and
  ##   `ci/test/web-renderer-mounts.sh`'s arm R now measures the proportions
  ##   the new declaration states — 20/55/25 over FILES, the editor and
  ##   CONSTRAINTS — and additionally asserts that the TESTS tab is present in
  ...
  of EditMode, QuickEditMode, InteractiveEditMode:
    ...
    # has no EVENT LOG to nest CONSTRAINTS under (`modeHiddenContentIds`), and
    # a circuit's cost is a thing you act on while editing the circuit.
    @[
      @[ord(Content.TestResults), ord(Content.Filesystem)]
    ]
