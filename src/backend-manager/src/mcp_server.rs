...
- `trace.add_watchpoint(name: str) -> int` - Watch a variable for VALUE CHANGES, returns ID.
  `continue_forward()` stops at the first later step where its recorded value differs.
  - **Refused** (reason on `TraceError.refusal`): non-identifiers (`obj.field`, `arr[0]`, `*ptr`);
    reads (a recording stores values, not accesses); conditions; unrecorded names; MCR/recreator.
