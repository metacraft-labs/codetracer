...
pub struct DapInitResult {
    ...
    /// Non-response events observed during initialization that still need
    /// normal client routing after the replay backend is installed.
    pub startup_events: Vec<Value>,
}
...
pub async fn run_dap_init(
    ...
    // Detect the trace events file: prefer trace.bin over trace.json, then
    // CTFS containers generated by materialized recorders.
    let trace_file = if trace_folder.join("trace.bin").is_file() {
        std::path::PathBuf::from("trace.bin")
    } else if trace_folder.join("trace.json").is_file() {
        std::path::PathBuf::from("trace.json")
    } else {
        let mut ct_files = std::fs::read_dir(trace_folder)
            .ok()
            .into_iter()
    ...
    Ok(DapInitResult {
        capabilities,
        stopped_event,
        startup_events: collected_events,
    })
}
