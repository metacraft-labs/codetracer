...
proc findRawTraceWithCodetracer(app: ElectronApp, traceId: langstring): Future[cstring] {.async.} =
  ## M-REC-2: ``traceId`` is now the canonical UUIDv7 recording_id
  ## (``langstring``).  Pass it through to ``ct trace-metadata --id=<uuid>``.
...
proc findTraceWithCodetracer*(app: ElectronApp, traceId: langstring): Future[Trace] {.async.} =
