...
# ---------------------------------------------------------------------------
# THE THIRD REGISTRY IS NOT HERE, AND THAT IS THE POINT
# ---------------------------------------------------------------------------
#
# `ui/editor.nim`'s `commands` table is the third claimant on a keyboard chord.
# `delegateShortcuts` hands every entry to `monacoEditor.addCommand`, on every
# Monaco instance, in every build, and until it was enumerated nothing could
# see a collision between it and either of the two registries above:
# `conflictList` sees collisions between two YAML entries, and `hardBoundChords`
# enumerates the `Mousetrap.bind` literals. A `commands` entry is neither.
#
# It IS enumerated now — in `frontend/tests/shortcut_bindings_test.nim`, as
# `MONACO_COMMAND_CHORDS` beside `canonicalChord`, `PERMITTED_MONACO_DOUBLE_CLAIMS`
# and `KNOWN_MONACO_SEMANTIC_COLLISIONS`. This pointer exists because a reader
# of this file would otherwise reasonably conclude there are two registries.
#
# WHY THERE AND NOT HERE, since `hardBoundChords` above is the same kind of
# object and lives in the product. The difference is that `hardBoundChords` has
# a RUNTIME CONSUMER: `hardBindShadowedActions` reads it and `configureShortcuts`
# logs what that returns, so the product itself uses the list. The Monaco
# registry has no such consumer — it is a description of `ui/editor.nim`'s
# source text, checked against that source text by `staticRead`, and asserted
# over. Putting it here would export four symbols that no product module reads,
# which `ci/test/frontend-reachability.sh` counts and which
# `ci/test/frontend-reachability-allowlist.txt` refuses to excuse in exactly
# these words: "'The test covers it.' That is the defect, not the exemption."
#
# The guard is not weakened by the move. It still `staticRead`s `ui/editor.nim`
# and requires the registry and the source to name the same chords in both
# directions, so the table cannot go stale; what changes is only that a
# statement ABOUT the product no longer pretends to be a capability OF it.
