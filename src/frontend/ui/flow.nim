...
proc calculateFlowLineLeftOffset(self:FlowComponent, flowLine: FlowLine): int =
  case self.data.config.`real-flow-ui`:
...
proc moveStepValuesInVisibleArea(self: FlowComponent) = # TODO: needs refeactoring
  let flowMode = 
    ($self.data.config.`real-flow-ui`)
      .substr(4, ($self.data.config.`real-flow-ui`).len - 1)
      .toLowerAscii()
...
proc switchFlowUI*(self: FlowComponent, flowUI: FlowUI) =
  if self.data.config.`real-flow-ui` == flowUI:
    return
  ...
  self.data.config.`real-flow-ui` = flowUI
  self.data.redraw()
...
proc updateLoopBackground(self: FlowComponent, loopIndex: int) =
  ...
  if self.data.config.`real-flow-ui` != FlowMultiline:
...
proc renderFlow*(self: FlowComponent, position: int, stepCount: int): VNode =
  ...
  for values in valueLines:
    if self.data.config.`real-flow-ui` == FlowMultiline and not self.multilineZones[position].variables[values[0]]:
      continue
...
proc switchFlowType*(self: FlowComponent, flowType: FlowUI) =
  if self.data.config.`real-flow-ui` != flowType:
    self.resetFlow()
    self.data.config.`real-flow-ui` = flowType
    self.recalculateAndRedrawFlow()
    self.updateFlowDom()
