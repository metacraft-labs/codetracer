...
proc initLayout*(initialLayout: GoldenLayoutResolvedConfig,
  ...
  layout.on(cstring"stackCreated") do (ev: Event):
    ...
    #
    # Guard against duplicate subscriptions — ``stackCreated`` can fire
    # multiple times for the same stack during layout restructuring (e.g.
    # after a component is detached).  Without this guard every subsequent
    # ``stackCreated`` would add another listener, causing one pin click to
    # create multiple auto-hide panels.
    let stack = ev.toJs.target
    let alreadySubscribed = not stack.autoHidePinSubscribed.isUndefined and
                            stack.autoHidePinSubscribed.to(bool)
    if not alreadySubscribed:
      stack.autoHidePinSubscribed = true.toJs
