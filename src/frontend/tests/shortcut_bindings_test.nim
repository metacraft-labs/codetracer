...
# between two of its entries) and `ui/shortcut_labels.nim`'s `hardBoundChords`
# (every `Mousetrap.bind("...")` literal, checked against the source by the
# case below that walks it).
#
...
#
# THE COST OF THE INVISIBILITY IS ON THE RECORD TWICE, in the product's own
...
# `hardBoundChords` is: hand-maintained, and guarded against going stale by the
# case below, which `staticRead`s `ui/editor.nim`, extracts every literal key of
# the `commands` table and requires the two lists to name the same chords in
# both directions.
#
# WHY IT LIVES IN THE TEST MODULE AND NOT BESIDE `hardBoundChords`, which is the
# same kind of object and does live in the product. `hardBoundChords` has a
# RUNTIME CONSUMER — `hardBindShadowedActions` reads it and `configureShortcuts`
# logs what that returns — so it is a capability the product uses. This registry
# has no such consumer: it is a statement ABOUT `ui/editor.nim`'s source text,
# checked against that source text, and read by nothing but the two cases below.
# Exporting it from `ui/shortcut_labels.nim` put four symbols in a product
# module that no product module reads, which is the twelve-times-found defect
# `ci/test/frontend-reachability.sh` counts — and its allow-list refuses that
# exemption in these exact words: "'The test covers it.' That is the defect, not
# the exemption." Moving it here removes the finding by removing the false
# claim, rather than by excusing it or by raising the ceiling.
