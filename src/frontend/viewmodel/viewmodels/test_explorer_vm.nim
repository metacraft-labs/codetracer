...
proc buildDiscoverFileCommand*(workspaceRoot, file: string): CtTestCommand =
  command("ct", "test", "discover", "--workspace", workspaceRoot, "--file", file, "--json")

proc buildRunSelectorCommand*(selector: string): CtTestCommand =
  command("ct", "test", "run", "--selector", selector, "--json-events")

proc buildRecordSelectorCommand*(selector: string; policy: TraceOpenPolicy): CtTestCommand =
  command("ct", "test", "record", "--selector", selector, "--json-events", "--open-policy=" & $policy)

proc buildRunFileCommand*(file: string): CtTestCommand =
  command("ct", "test", "run", "--file", file, "--json-events")

proc buildRecordFileCommand*(file: string; policy: TraceOpenPolicy): CtTestCommand =
  command("ct", "test", "record", "--file", file, "--json-events", "--open-policy=" & $policy)
...
proc createTestExplorerViewModel*(
    workspaceRoot: string;
    ctTest: CtTestService;
    traceOpen: TraceOpenService): TestExplorerViewModel =
...
proc actionsFor(catalog: TestCatalog; policy: TraceOpenPolicy): seq[EditorTestAction] =
  result = @[]
...
proc ingestFileCatalog*(vm: TestExplorerViewModel; file: string; catalog: TestCatalog;
                        policy = topCurrentTab) =
...
proc editorActionsForFile*(vm: TestExplorerViewModel; file: string): seq[EditorTestAction] =
...
proc handleEvent*(vm: TestExplorerViewModel; event: TestEvent;
  ...
  of tekRecordingCreated:
    if event.trace.isSome:
      ...
      let policy = vm.runOpenPolicies.getOrDefault(event.runId, defaultOpenPolicy)
