  ...
  @[
    ...
    RuntimeAsset(
      ...
      absenceBehaviour:
        ...
        "stepped through: this page was published without the replay " &
        "engine, and says so instead of opening a debugger that answers " &
        "nothing"),
    RuntimeAsset(
      id: replayEngineGlueId, path: replayEngineGluePath, mode: damFetched,
      required: false, fetchedBy: fcReplayEngine,
      absenceBehaviour:
        "the replay engine cannot start, so a run made here can be " &
        "summarised but not stepped through"),
    RuntimeAsset(
    ...
    RuntimeAsset(
      ...
      required: false, fetchedBy: fcNoirWasmWorker,
      absenceBehaviour:
        "an Aztec contract compiled here cannot be turned into AVM " &
        "bytecode, so it stops at the compiled artifact")]
...
proc parseDeploymentDescriptor*(text: string): DeploymentDescriptor =
  ...
  for entry in languageOrigins:
    ...
    var known = false
    for row in knownLanguageEntries:
      if row.entry == declared.language: known = true
...
proc renderEntryDocument*(descriptor: DeploymentDescriptor): string =
  ...
  ## update.
  ##
  ## ## The `<title>` is the NEUTRAL product name, and that is deliberate
  ##
  ## This one document is served at every address the deployment has — the
  ## rewrite table above points all of them at it — so its title is a single
  ## string for `ide.codetracer.com/`, for `/noir`, and for `noirstudio.dev`.
  ## It used to be `CodeTracer — Noir Studio`, which is the shape of answer a
  ## single string forces: two products named at once, so that the page is
  ## wrong wherever the reader is. Every bookmark taken on the neutral root
  ## carried the other product's name.
  ##
  ## A document cannot be right here, because it does not know the origin it
  ## will be requested from. What it can be is right about the deployment's own
  ## canonical root — `neutralProductName`, from the module that owns the names
  ## — and correctable by the arm that does know. `ui_js.startWebRenderer`
  ## resolves the entry and sets `document.title` from
  ## `web_entry.productNameFor`, so the page is correct from the first frame a
  ## user is shown anything.
  ##
  ## Interpolated rather than spelled, so a build cannot ship a title the
  ## product does not answer to.
  ##
...
<title>""" & neutralProductName & """</title>
<!--
...
<div id="codetracer-boot">Starting """ & neutralProductName & """&hellip;</div>
<div id="codetracer-renderer">Opening your workspace&hellip;</div>
<!--
