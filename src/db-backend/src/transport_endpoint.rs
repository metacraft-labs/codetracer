...
///
/// The optional extension is appended as `.{extension}` when non-empty.
pub fn unix_socket_path_for_pid(socket_dir: &Path, base_name: &str, pid: usize, extension: Option<&str>) -> PathBuf {
...
mod tests {
    ...
    fn endpoint_enum_preserves_payload() {
        ...
        assert_eq!(
            endpoint,
            DapEndpoint::WindowsNamedPipe(r"\\.\pipe\ct_dap_socket_12".to_string())
        );
    }
}
