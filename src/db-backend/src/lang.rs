...
pub enum Lang {
    ...
    // blockchain-VM languages, whose traces are materialized and read here
    // rather than stepped through a native frame.  They are NOT absent from
    // the Nim frontend enum — `src/common/common_lang.nim` declares all 40 of
    // these variants at the same ordinals, and
    // `src/tests/cli/lang_enum_contract_test.nim` asserts the two lists are
    // the same length, name for name.  (This comment used to claim they were
    // "not in the Nim frontend enum", which was false in a way nothing
    // checked.)
    //
...
impl Lang {
    ...
    /// `wire_names_match_native_backend_spellings` below pins down.
...
/// (`crate::query::ReplayQuery`).  The DAP-facing structs keep `serde_repr`
/// because the Nim frontend still sends `lang` as an integer there.
pub mod lang_wire {
    use super::Lang;
