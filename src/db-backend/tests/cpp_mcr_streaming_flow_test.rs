...
//! and values.
//!
//! The C++ program is built with `g++` by ct-native-replay; the test
...
#[test]
fn cpp_mcr_streaming_flow_variables_and_values() {
    ...
    let source_path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("test-programs/cpp/cpp_flow_test.cpp");
    assert!(
        source_path.exists(),
    ...
    // --- record under MCR ---
    let recording =
        TestRecording::create_mcr(&source_path, Language::Cpp, "mcr", &ct_native_replay).expect("MCR recording failed");
