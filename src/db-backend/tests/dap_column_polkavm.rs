//! M-polkavm — column-aware DAP breakpoints on PolkaVM-shaped traces.
//!
//! Spec: codetracer-specs/Planned-Features/Column-Aware-Navigation.status.org
//! §M1 (PolkaVM adaptation).  Companions:
//!   * `dap_column_breakpoint.rs` (JS-shaped reference)
//!   * `codetracer-polkavm-recorder/tests/test_column_aware_steps.rs`
//!     (recorder-side regression — proves blob → trace round-trips
//!     three distinct columns on one source line).
//!
//! Pins the DAP-layer half of the contract: fed a PolkaVM-shaped
//! materialized trace, `Handler::setBreakpoints` / `step(Continue)`
//! honour recorded `(line, column)` tuples.  Fixture mirrors the
//! recorder repo's `let a: u32 = 1; let b: u32 = 2; let c: u32 = 3;`
//! line at cols {1, 17, 33}; built synthetically in-memory so the test
//! runs in the codetracer cargo matrix with zero sibling-repo
//! dependency.
//!
//! Compile + run: cd src/db-backend && cargo test --test dap_column_polkavm
...
// ── Fixture ─────────────────────────────────────────────────────────────────
// Mirrors the recorder repo's `build_multi_stmt_blob` line=2 / col∈{1,17,33}
// fixture (`let a: u32 = 1; let b: u32 = 2; let c: u32 = 3;`).

const RECORDED_FILE: &str = "column_aware_multi_stmt.polkavm";
const SOURCE_FILE: &str = "column_aware_multi_stmt.rs";

const FIXTURE_LINE: i64 = 2;
const COL_LET_A: i64 = 1;
const COL_LET_B: i64 = 17;
const COL_LET_C: i64 = 33;
/// Later, line-only step — legacy-fallback target (hypothetical `ret`).
const RET_LINE: i64 = 3;

fn build_trace(trace_dir: &PathBuf) -> (Arc<dyn TraceReader>, String) {
    // The recorder embeds the user-edited source path (the `.rs`
    // fixture), not the blob path, into the trace's path table.
    let recorded = trace_dir.join(SOURCE_FILE).display().to_string();
    let mut db = Db::new(trace_dir);
    // PathId(0) sentinel, PathId(1) source file.
    db.paths.push(String::new());
    db.paths.push(recorded.clone());
    db.path_map.insert(recorded.clone(), PathId(1));
    db.types.push(TypeRecord {
        kind: TypeKind::Int,
        lang_type: "u32".to_string(),
    ...
    // Steps 0..2 — three columns on FIXTURE_LINE (three `let`s).
    // Step 3 — RET_LINE col 1 — line-only fallback target.
    ...
    let _ = std::fs::write(trace_dir.join(RECORDED_FILE), b"<synthetic polkavm fixture>");
...
fn fresh_trace_dir(tag: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("dap_column_polkavm_{tag}_{}", std::process::id()));
    if !dir.exists() {
...
/// Boot a `Handler` over a fresh trace at step 0.  Returns `(handler, recorded_path)`.
...
/// STRICT — `setBreakpoints {line: FIXTURE_LINE, column: COL_LET_B}`
/// stops the next Continue at the step recorded at col 17 and the
/// response echoes the bound column back so DAP clients can anchor the
/// gutter glyph.
#[test]
fn polkavm_column_breakpoint_stops_at_recorded_column() {
...
/// STRICT — a column-aware breakpoint at the THIRD column on the
/// fixture line MUST skip steps recorded at cols 1 and 17 and land at
/// step 2.  Without this assertion an engine that stores the column but
/// quietly matches line-only would pass the first test by luck (step 1
/// happens to be the second step on the line).
#[test]
fn polkavm_column_breakpoint_skips_same_line_other_columns() {
...
/// STRICT — legacy line-only breakpoints (no column on the request, the
/// shape a no-DWARF PolkaVM trace produces at every step) MUST keep
/// working.  Continue with `{line: RET_LINE}` stops at step 3.  Pins
/// the back-compat invariant codified recorder-side by
/// `test_column_aware_flag_set_even_without_dwarf_columns`.
#[test]
fn polkavm_line_only_breakpoint_still_stops_at_line_start() {
