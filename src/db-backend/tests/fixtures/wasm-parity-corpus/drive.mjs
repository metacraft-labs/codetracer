// Parity corpus — headless driver.
//
// Loads one corpus page in headless Chromium and waits for its own
...
// truncate it into something that looks valid and is missing records.
//
//     node drive.mjs <preview-port> <record-web-port> <module-name>
//
// Environment:
//   CORPUS_HEADFUL=1   show the browser window
...
const RECORD_WEB_PORT = Number(process.argv[3] || 9232);
const MODULE_NAME = process.argv[4];
if (!MODULE_NAME) {
  console.error("usage: node drive.mjs <preview-port> <record-web-port> <module>");
  process.exit(2);
}
...
const browser = await chromium.launch({
  headless: process.env.CORPUS_HEADFUL !== "1",
...
try {
  ...
  if (!status.startsWith("done ")) {
    throw new Error(`the page did not finish cleanly: ${status}`);
  }
  console.log(`[drive] ${MODULE_NAME}: ${status}`);
