# Expected Origin Chain — rust / parameter_pass

**Query target:** local `local` at the `println!` line inside `receive`.
...
hop 2: target=value   rhs=7              OriginKind=Literal       terminator=Literal(i32, value=7)
```
