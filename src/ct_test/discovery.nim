...
type
  ...
  DiscoveryCache* = ref object
    entries: Table[string, TestCatalog]
...
proc newDiscoveryCache*(): DiscoveryCache =
  DiscoveryCache(
    entries: initTable[string, TestCatalog](),
...
proc configFingerprint(workspaceRoot: string; configFiles: seq[string]): string =
...
proc discoverFileCached(
  ...
  let
    ...
    key = cacheKey(workspaceRoot, provider.provider.info.id, filePath, fileHash, configHash)
...
proc validateRequest*(request: DiscoverRequest): seq[TestDiagnostic] =
  ...
  if request.workspaceRoot.len == 0:
    ...
    result.add diagnostic(dsError, "invalid workspace: directory does not exist: " & request.workspaceRoot)
  ...
  if request.scope == dskFile:
    if request.file.len == 0:
      ...
      result.add diagnostic(dsError, "invalid file: file does not exist: " & request.file, request.file)
...
proc parseDiscoverArgs*(args: seq[string]): ProviderResult[DiscoverRequest] =
  ...
  while i < args.len:
    ...
    of "--json":
      ...
      diagnostics.add diagnostic(dsError, "unknown discover argument: " & args[i])
    inc i

  if request.workspaceRoot.len == 0 and request.scope == dskFile and request.file.len > 0:
  ...
  for provider in registry.providers:
    ...
    if not detected.value:
      result.diagnostics.add diagnostic(
        dsWarning,
        "unsupported provider: " & provider.provider.info.id & " did not detect a compatible framework")
      continue
    ...
    of dskFile:
      if not provider.provider.info.capabilities.canDiscoverFile:
        result.diagnostics.add diagnostic(
          dsWarning,
          "unsupported provider: " & provider.provider.info.id & " cannot discover files",
          request.file)
        continue
      let providerResult = cache.discoverFileCached(provider, request.workspaceRoot, request.file)
    ...
    of dskWorkspace:
      if not provider.provider.info.capabilities.canDiscoverProject:
        result.diagnostics.add diagnostic(
          dsWarning,
          "unsupported provider: " & provider.provider.info.id & " cannot discover workspaces")
        continue
      let providerResult = provider.provider.discoverProject(request.workspaceRoot)
      for item in providerResult.diagnostics:
        result.diagnostics.add item
      result.catalogs.add providerResult.value

  if supportedProviders == 0:
    result.diagnostics.add diagnostic(dsError, "no supported test providers detected for workspace")
...
proc fakeItem(providerId, workspaceRoot, filePath: string; line: int): TestItem =
...
proc fakeFileCatalog(providerInfo: TestProviderInfo; workspaceRoot, filePath: string): TestCatalog =
  ...
  TestCatalog(
    ...
    items: items,
    diagnostics: @[diagnostic(dsInfo, "stale parser/cache fallback: M1 fake provider uses pattern-based fixture discovery", filePath)])

proc fakeProjectFiles(workspaceRoot: string): seq[string] =
  result = @[]
  for path in walkDirRec(workspaceRoot):
    if fileExists(path) and path.endsWith(".fake") and splitFile(path).name != "ct-test":
      result.add path
  result.sort(system.cmp[string])

proc newFakeProviderRegistry*(counters: FakeProviderCounters): ProviderRegistry =
  ...
  provider.detect = proc(projectRoot: string): ProviderResult[bool] {.gcsafe.} =
    ...
    ProviderResult[bool](diagnostics: @[], value: fileExists(projectRoot / "ct-test.fake"))
  provider.discoverFile = proc(projectRoot, file: string): ProviderResult[TestCatalog] {.gcsafe.} =
    ...
    counters.discoverFileCalls[normalized] = counters.discoverFileCalls.getOrDefault(normalized, 0) + 1
  ...
  let unsupportedInfo = TestProviderInfo(
    ...
    version: "m1",
    capabilities: baseCapabilities(canDiscoverProject = false, canDiscoverFile = false))
  var unsupported = TestProvider(info: unsupportedInfo)
  unsupported.detect = proc(projectRoot: string): ProviderResult[bool] {.gcsafe.} =
    ProviderResult[bool](
      diagnostics: @[diagnostic(dsInfo, "unsupported provider probe completed: m1-unsupported")],
      value: false)
