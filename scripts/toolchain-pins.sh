...
devshell_init() {
	...
	# The generated parser. `nix/shells/main.nix`'s shellHook runs
	# `just generate` in `libs/tree-sitter-nim`, producing a 42 MB UNTRACKED
	# `src/parser.c`. Checked separately from the submodule state because the
	# two can disagree: a submodule can be initialised and the parser still
	# never generated, and the failure that follows names a C file.
	#
	# Guarded on the DIRECTORY EXISTING, because CI clones with
	# `submodules: false` and skips the regen deliberately — firing there would
	# be this guard failing a lane for a file that lane is designed not to have.
	if [ -d "$REPO_ROOT/libs/tree-sitter-nim" ]; then
		if [ -f "$REPO_ROOT/libs/tree-sitter-nim/src/parser.c" ]; then
			...
			printf '           It is generated by `just generate` from the dev shell hook and is\n'
			printf '           not tracked, so a checkout the shell never ran in does not have it.\n'
		fi
	fi
