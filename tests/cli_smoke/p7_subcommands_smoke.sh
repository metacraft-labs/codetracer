...
if [ ! -x "$CT" ]; then
	# A missing prerequisite is a hard failure, not a silent pass. The old
	# "SKIP-discipline" comment justified exit 0 with "real CI runs this test
	# only after a successful build" — an assumption nothing checked, and one
	# that silently voids the whole test the moment the build path moves.
